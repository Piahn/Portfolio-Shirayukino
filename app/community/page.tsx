  ...
  return (
    <div className="w-full transition-colors duration-300">
      ...
      <div className="px-[21px] py-12 sm:px-8 sm:py-16">
        <div className="mx-auto max-w-[1220px]">
          <div className="flex items-end justify-between mb-8 pb-4 border-b border-[#e7e3df] dark:border-[#1a2840]">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#8f8b86] dark:text-[#7f93b0]">
                Official Channels
              </p>
              <h2 className="text-2xl sm:text-3xl font-semibold tracking-[-0.02em] text-black dark:text-white mt-1">
                Where our circle gathers
              </h2>
            </div>
            <span className="hidden sm:block font-mono text-xs text-gray-500 dark:text-gray-400">
              4 Active Platforms
            </span>
          </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div className="flex items-center gap-2.5">
                        <div className="flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-[#15233c] border border-[#e7e3df] dark:border-[#1e3456] shadow-sm text-xl text-[#142d55] dark:text-[#E5BA68]">
                          <Icon />
                        </div>
                        <div>
                    ...
                    <p className="text-xs sm:text-[13.5px] leading-relaxed text-gray-600 dark:text-gray-300 mt-3">
                      {channel.description}
                    </p>

                    <div className="mt-5 pt-4 border-t border-[#e2ddd7] dark:border-[#192b45] space-y-2">
          ...
          <div className="mt-16 rounded-[22px] border border-[#e7e3df] dark:border-[#1a2840] bg-[#f8f5f2] dark:bg-[#0c1524] p-7 sm:p-10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 pb-4 border-b border-[#e7e3df] dark:border-[#1a2840]">
              <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {COMMUNITY_MILESTONES.map((m, idx) => (
                <div
                  key={idx}
                  className={`flex flex-col justify-between rounded-[18px] p-5 border transition-all ${
                    m.completed
                ...
                >
                  ...
                  <div className="mt-4 pt-3 border-t border-[#e7e3df] dark:border-[#1a2840] flex items-center gap-1.5 text-[11px] font-mono text-gray-500 dark:text-gray-400">
                    <IoShieldCheckmarkOutline className={m.completed ? "text-emerald-500" : "text-gray-400"} />
                    <span>{m.completed ? "Unlocked for all players" : "Next goal unlock"}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-14">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {GUIDELINES.map((g, i) => (
                <div
                  key={i}
                  className="rounded-[18px] border border-[#e7e3df] dark:border-[#1a2840] bg-[#f8f5f2] dark:bg-[#0c1524] p-5 sm:p-6"
                >
                  <div className="flex items-center gap-2.5 font-mono text-xs font-semibold text-[#142d55] dark:text-[#E5BA68] mb-2">
                    <span className="w-5 h-5 rounded-full bg-white dark:bg-[#15233c] border border-[#e7e3df] dark:border-[#1e3456] flex items-center justify-center text-[10px]">
                      {i + 1}
                    </span>
                    <span>{g.title}</span>
                  </div>
          ...
          <div className="mt-14 rounded-[22px] bg-gradient-to-r from-[#142d55] to-[#0a172c] dark:from-[#0e1c33] dark:to-[#08101e] p-7 sm:p-9 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border border-[#23457a] dark:border-[#1e3b68] shadow-md">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white dark:bg-[#E5BA68] text-[#142d55] dark:text-[#08101e] text-xs font-mono font-semibold uppercase tracking-wider transition hover:bg-gray-100 dark:hover:bg-[#f0c878] shrink-0 shadow-sm group cursor-pointer"
            >
